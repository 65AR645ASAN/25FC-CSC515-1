...
def fetch_images(origin_img_path, destination_img_path):
    ...
    if origin_img is None:
        ...
        # Check if destination_img is None:
...
def create_fusion_overlay(vertical_size, horizontal_size):
    ...
    cv2.ellipse(fusion_overlay, (horizontal_size // 2, vertical_size // 2), (horizontal_size // 2, vertical_size // 2),
...
user_portrait = os.path.join(code_location,
                             "bearded_selfie_in_car.jpeg")  # Path to personal portrait (updated file type)
...
try:
    source_photo, target_photo = fetch_images(user_portrait, web_portrait)
    source_mono = prepare_grayscale(source_photo)
    target_mono = prepare_grayscale(target_photo)

    face_detector = setup_recognizer()

    source_detections = find_facial_areas(source_mono, face_detector)
    target_detections = find_facial_areas(target_mono, face_detector)
    ...
    result_file = os.path.join(code_location, "blended_deepfake_result.jpg")
