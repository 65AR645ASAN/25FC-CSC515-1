...
def generate_and_store_visual_summary(stages_list, labels_list, output_file):
    # Initialize a wide figure for horizontal arrangement with more height for titles
    plot_lib.figure(figsize=(18, 8))
    for pos in range(len(stages_list)):
        ...
        plot_lib.title(labels_list[pos], fontsize=10)
        plot_lib.axis('off')
    # Optimize spacing and save with high resolution
    plot_lib.subplots_adjust(top=0.95)
